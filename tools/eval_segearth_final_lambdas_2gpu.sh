...
LAMBDAS=("0" "-0.02" "-0.05" "-0.08" "-0.1" "-0.2" "-0.3")

for L in "${LAMBDAS[@]}"; do
  TAG=$(echo "$L" | sed 's/-/m/g; s/\./p/g')

  OUT_DIR=gba_lamda_test/output_eval_vitb_noise_2gpu_segearth_final_lambda_${TAG}
  LOG_FILE=logs/eval_vitb_noise_2gpu_segearth_final_lambda_${TAG}.log
